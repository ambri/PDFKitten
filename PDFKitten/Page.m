...
- (void)didDoubleTap:(UITapGestureRecognizer *)recognizer {
    ...
    CGFloat fullPageScale = MIN(hScale, vScale);
    CGFloat fillPageScale = MAX(hScale, vScale);
    if (self.zoomScale < (fillPageScale-fullPageScale)/2 + fullPageScale) {
        [self setZoomScale:fillPageScale animated:YES];
    } else {
        [self setZoomScale:fullPageScale animated:YES];
    }
}
